...
class TestsCreateSpends:
    ...
    def test_create_spend(self, page: Page, authenticated_user):
        ...
        spendings_page.spending_row(
            spending_category, spending_amount, spending_date
        ).wait_for()

        # Удаляем трату и категорию
        spendings_page.delete_spending(
            spending_category, spending_amount, spending_date
        )
...
class TestsEditeSpends:
    ...
    def test_edit_spend(self, page: Page, created_spend):
        ...
        spendings_page.edit_spending(
            ...
            spending_day,
            created_spend.description,
            new_spending_amount,
            new_spending_currency,
            new_spending_category,
            new_spending_date,
            new_spending_description,
        )
...
class TestsSearchSpends:
    ...
    def test_search_spending_by_category(self, page: Page, created_spend, authenticated_user):
        ...
        another_category = fake.word() + "Test123"
        now = datetime.now(timezone.utc)
