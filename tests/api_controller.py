...
def get_categories():
    ...
    allure.attach(
        result.text, name="Response Body", attachment_type=allure.attachment_type.JSON
    )
...
def create_category(authenticated_user) -> tuple[str, uuid.UUID]:
    ...
    category_data = {"name": category_name, "username": username, "archived": False}
    result = requests.post(
        f"{base_url}categories/add", json=category_data, headers=headers
    )
    allure.attach(
        str(headers),
        name="Request Headers",
...
def create_spending(
    ...
    allure.attach(
        str(data), name="Request Body", attachment_type=allure.attachment_type.JSON
    )
    allure.attach(
        str(result.status_code),
        name="Status Code",
